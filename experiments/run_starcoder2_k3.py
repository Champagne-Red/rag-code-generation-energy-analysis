# run_starcoder_k3_windows.py - StarCoder2:instruct with RAG (k=3)
...
# Generation params
GEN_PARAMS = {"num_predict": 160, "temperature": 0.0, "top_p": 1.0, "keep_alive": 300}

# Reference caps (for retrieved docs)
REF_MAX_TOKENS_PER_DOC = 240
REF_MAX_TOTAL_TOKENS = 900


# ===================== CORPUS LOADING =====================
def load_corpus_docs() -> List[str]:
    docs = []
    for p in glob.glob(os.path.join("corpus", "**", "*.txt"), recursive=True):
        try:
...
def retrieve(bm25, docs, query: str, k: int) -> List[str]:
    if k == 0 or not bm25:
        return []
...
# ===================== PROMPT BUILDING =====================
def _truncate_to_tokens(txt: str, limit: int) -> str:
    words = txt.split()
    if len(words) <= limit:
        return txt
...
def _clamp_refs(refs: List[str]) -> str:
    out = []
    used = 0
    for r in refs:
        rem = max(0, REF_MAX_TOTAL_TOKENS - used)
        if rem == 0:
            break
        take = min(REF_MAX_TOKENS_PER_DOC, rem)
        clipped = _truncate_to_tokens(r, take)
        used += len(clipped.split())
        out.append(clipped)
    block = "\n---\n".join(out)
    return "\n".join(f"# REF: {line}" for line in block.splitlines()) + ("\n" if block else "")


def build_prompt(task_text: str, signature: str, refs: List[str], model: str) -> str:
    ref_block = _clamp_refs(refs) if refs else ""
    ...
    if model.startswith("starcoder2"):
        return (
            ...
            f"{ref_block}"
            f"{signature}\n"
            f"    # Complete this function and return the correct result\n"
        )
    return ""
...
def write_results_header(path: str):
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        csv.writer(f).writerow([
            ...
            "t_retrieval_s", "t_generate_s", "t_total_s", "status",
...
def summarize_results(csv_path: str, model: str, k: int):
    ...
    print(f"{'=' * 80}")
    print(f"{'Source':<12} {'Pass':<6} {'Retr(s)':<10} {'Gen(s)':<10} {'Total(s)':<10} {'Tokens':<8} {'Count':<6}")
    print(f"{'-' * 80}")

    for source, grp in sorted(by_source.items()):
        pass_rate = sum(int(x["pass_at_1"]) for x in grp) / len(grp)
        med_retr = statistics.median(float(x["t_retrieval_s"]) for x in grp)
        med_gen = statistics.median(float(x["t_generate_s"]) for x in grp)
        med_tot = statistics.median(float(x["t_total_s"]) for x in grp)
        total_tokens = sum(int(x.get("gen_tokens", 0)) for x in grp)
        print(
            f"{source:<12} {pass_rate:.3f}  {med_retr:.6f}  {med_gen:.6f}  {med_tot:.6f}  {total_tokens:<8} {len(grp):<6}")

    all_pass = sum(int(r["pass_at_1"]) for r in rows) / len(rows)
    all_retr = statistics.median(float(r["t_retrieval_s"]) for r in rows)
    all_gen = statistics.median(float(r["t_generate_s"]) for r in rows)
    all_tot = statistics.median(float(r["t_total_s"]) for r in rows)
    all_tokens = sum(int(r.get("gen_tokens", 0)) for r in rows)
    print(f"{'-' * 80}")
    print(f"{'ALL':<12} {all_pass:.3f}  {all_retr:.6f}  {all_gen:.6f}  {all_tot:.6f}  {all_tokens:<8} {len(rows):<6}")
    print(f"{'=' * 80}\n")
...
def main_run(run_index: int):
    print(f"\n{'=' * 70}")
    print(f"EXPERIMENT: {MODEL} | k={K} (WITH RAG) | RUN {run_index}/7")
    print(f"{'=' * 70}\n")

    out_csv = os.path.join(OUT_DIR, f"starcoder_k{K}_run{run_index}_results.csv")

    # Power monitor setup - skip for run 1 (warmup)
    power_monitor = None
    if run_index > 1:
        power_log = os.path.join(OUT_DIR, f"starcoder_k{K}_run{run_index}_power.csv")
    ...
    bm25, raw_docs = build_bm25(docs)
    print(f"CORPUS: Loaded {len(raw_docs)} documents\n")

    print(f"Total tasks: {len(all_tasks)} (MBPP: {len(mbpp_tasks)}, HumanEval: {len(humaneval_tasks)})\n")
    ...
    for i, task in enumerate(all_tasks, 1):
        ...
        refs = retrieve(bm25, raw_docs, task["text"], K)
        prompt = build_prompt(task["text"], task["signature"], refs, MODEL)
        ...
        t_total = t_retrieval + t_generate
        ...
        append_result_row(out_csv, [
            ...
            int(passed), f"{t_retrieval:.6f}", f"{t_generate:.6f}",
            f"{t_total:.6f}", status, prompt_tokens, gen_tokens
        ])

        status_str = " | TIMEOUT" if status == "timeout" else (" | ERROR" if status == "error" else "")
        print(f"[{i:3d}/{len(all_tasks)}] {task['task_id']:<20} | pass={int(passed)} | "
              f"retr={t_retrieval:.4f}s | gen={t_generate:.3f}s{status_str}", flush=True)
...
if __name__ == "__main__":
    for i in range(1, 8):  # 7 runs total
        ...
        if i < 7:
            print(f"\n{'*' * 70}")
            print(f"Pausing 60 seconds before next run...")
            print(f"{'*' * 70}\n")
            time.sleep(60)
